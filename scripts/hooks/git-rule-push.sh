...
#     задачи, `main` либо ветка до правила (свой коммит с датой автора до T0);
#     ветка, уже лежащая на удалённом, не переименовывается;
...
while read -r _lref lsha rref rsha; do
    ...
    if [ -n "$name" ] && [ "$existed" = 0 ] && [ "$name" != main ] &&
        ...
        GIT_RULE_FINDINGS+=("ветка «$name»: новая ветка называется номером задачи (^[0-9]+\$), исключение одно — main; ветка до правила несёт свой коммит с датой автора до T0")
    fi
    if [ -n "$cannot" ]; then
...
echo "== правило git (scripts/hooks/git-rule.sh; T0 ${t0_line:-—}): ссылок $refs, новых коммитов $GIT_RULE_SEEN, из них с датой автора после T0 $GIT_RULE_AFTER_T0"
