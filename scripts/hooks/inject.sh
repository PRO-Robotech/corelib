...
# дефект сборки, vet, gofmt, пробы и линтера кладётся во вложенный пакет (у
# gofmt — ещё и в файл, не первый в обходе): проверка, суженная до корня или до
# части обхода, его не увидит — и утверждение разойдётся. У близнеца охват
# сверяется числами: файлы gofmt, пакеты и пробы go test, пакеты линтера.
# Подставной линтер исполняет ту часть
