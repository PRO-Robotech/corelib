...
# которой зависит вердикт о дереве, — какой конфиг прочитан, где кэш, какие
# пакеты осмотрены — и пишет её в журнал; проба сверяет журнал с деревом
# фикстуры и с ci.yml. «Тот же, что в ci.yml» — пин линтера и конфиг — берётся
# из исполняемых строк `run:` ci.yml, а не выписывается здесь второй раз.
#
# ЧЕГО ПРОБА НЕ РАЗЛИЧАЕТ: --timeout линтера и прочие его флаги, кроме
# --config/-c/--no-config, — подставной их глотает. Таймаут хука (5m) короче,
# чем в ci.yml (10m); истёкший таймаут — код 4 линтера, то есть красное, так что
# расхождение даёт лишь отказ там, где конвейер дождался бы, а не зелёное там,
# где он красный.
#
