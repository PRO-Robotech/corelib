...
// как у record). Одно место на все пути, где повтор замечается: выборку кода и
// токена обновления, ноль строк погашения и оборота. Поэтому ни один шаг моста
// после пометки повтора не записывает свой случай первым, и порядок шагов на
// каждом пути этого не решает.
