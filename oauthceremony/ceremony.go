...
// Утверждение о настройках держат две пробы, каждая свою половину:
//
...
//     значение под указателем, — которой первая проба не видит.
//
// Чего не судит ни одна из них: запись в поле того же значения, что там уже
// лежит; что делает со ссылочным содержимым код движка, получивший его от
// геттера; прочее состояние движка — обработчики, стратегию подписи.
// Одновременное использование всего этого проверяет только -race проба
// TestConcurrentExchangesOnAFreshCeremonyShareNoEngineState, и только на пути
// обмена.
