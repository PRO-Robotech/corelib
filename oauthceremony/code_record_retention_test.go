...
const (
	// codeRetentionLifespan — срок кода в пробах этого файла: короткий, чтобы
	// он истекал внутри пробы.
	codeRetentionLifespan = time.Second
	// codeRetentionWait — пауза между обменом и повтором. Больше срока с
	// запасом на округление движка до секунды (срок назначается
	// `Round(time.Second)`, то есть до полусекунды позже).
	codeRetentionWait = 2500 * time.Millisecond
)

func shortCodeLifespan(c *oauthceremony.Config) { c.AuthorizationCodeLifespan = codeRetentionLifespan }
...
// мига. И движок с этим согласен: нетронутый код, выданный тем же мигом,
// отвергнут `invalid_grant` — не повтором и не «кода нет». Текст отказа
// истечения не называет (у движка это `invalid_token` в подробностях), поэтому
// предпосылка судится записанным сроком, а не текстом.
func requireCodePastItsLifespan(t *testing.T, store *memoryPorts, signature string, untouchedErr error) {
	...
	if now := time.Now().UTC(); !expiresAt.Before(now) {
		t.Fatalf("НЕ ВЫПОЛНИЛОСЬ: срок кода %s не истёк к %s (пауза %s при сроке %s) — проба судила бы "+
			"повтор в пределах срока", expiresAt, now, codeRetentionWait, codeRetentionLifespan)
	}
	switch {
	case untouchedErr == nil:
		t.Fatalf("НЕ ВЫПОЛНИЛОСЬ: нетронутый код через %s при сроке %s обменян — движок срока не судит",
			codeRetentionWait, codeRetentionLifespan)
	case errors.Is(untouchedErr, oauthceremony.ErrAuthorizationCodeConsumed),
		...
		t.Fatalf("НЕ ВЫПОЛНИЛОСЬ: нетронутый код через %s отвергнут не сроком: случай %v (%v)",
			codeRetentionWait, oauthceremony.CodeOf(untouchedErr), untouchedErr)
	}
}
...
// выданный тем же мигом, после паузы отвергается истечением. Без неё пробы
// судили бы повтор в пределах срока (это предмет
...
func TestConsumedCodeRecordOutlivesTheCodeLifespan(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ceremony := newTestCeremony(t, store.ports(), shortCodeLifespan)
