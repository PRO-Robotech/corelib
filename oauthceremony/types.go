...
type SessionRecord struct {
	...
	// ExpiresAt — срок годности по каждому виду артефакта. Отсутствие
	// записи о виде означает «срок берётся из настроек церемонии» и
	// отличается от нулевого времени тем, что ключа нет.
...
type AuthorizationGrant struct {
	...
	// GrantedScopes — области, которые служба РЕШИЛА выдать. Область, не
	// входящая в запрошенные, отвергается движком.
	GrantedScopes []string

	// GrantedAudiences — получатели, которых служба решила выдать.
	GrantedAudiences []string

	// Claims — дополнительные утверждения в сеанс.
	Claims map[string]any

	// ExpiresAt — явные сроки годности по видам артефактов. Вид без
	// записи получает срок из настроек церемонии.
	ExpiresAt map[TokenKind]time.Time
}
