...
type AuthorizationCodeVault interface {
	...
	//
	// Третий исход обязан отдаваться, пока может жить семейство гранта, — и
	// после срока самого кода. Повтор узнаётся по записи, а не по сроку:
	// движок сверяет повтор раньше срока, и срока погашенного кода церемония
	// не судит. Запись, снятая по сроку кода, превратила бы повтор после срока
	// во второй исход — «кода нет», — и семейство осталось бы живым. Порог
	// уборки записи поэтому считается от tokenpolicy.MaxRefreshTokenFamilyTTL,
	// а не от tokenpolicy.MaxAuthorizationCodeTTL.
	//
...
type AccessTokenIssuer interface {
	...
	//
	// Контекст входа — grant.Session.SessionID, grant.Session.ACR и
	// grant.Session.AuthTime: сессия, уровень (`acr`) и момент (`auth_time`)
	// аутентификации, снятые на выдаче кода. У всего семейства они одни —
	// обмен и оборот их не пересчитывают, — и токен, несущий `acr` или
	// `auth_time`, берёт их ОТСЮДА: карта grant.Session.Claims ключей
	// контекста входа не несёт (церемония отвергает их при выдаче), и второго
	// источника у этих значений нет.
	//
