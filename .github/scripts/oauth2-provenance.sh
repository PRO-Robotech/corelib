...
# `.github/scripts/upstream-pins.sh`, общий с гейтом уведомлений об
# уязвимостях. Запись — модуль, версия, коммит, сумма, подкаталог апстрима
...
#   8. Каждый изменённый файл НЕСЁТ УВЕДОМЛЕНИЕ ОБ ИЗМЕНЕНИИ (Apache-2.0
#      §4(b)): последний шаг объявленного преобразования вставляет строку
#      NOTICE_LINE после идентификатора лицензии в каждый файл `.go` области,
#      который не совпал побайтово с файлом источника, и в каждый перенесённый
#      из источника (`moved-from`). Утверждается сравнением шага 6: изменённый
#      файл без строки и неизменённый со строкой — оба расхождение, КРАСНЫЙ.
#
...
# Уведомление об изменении (шаг 8). Строка одна на все файлы: какие именно
# изменения внесены — в манифесте, который она называет.
NOTICE_LINE="// Изменено PRO-Robotech (modified by PRO-Robotech): перечень изменений — internal/oauth2/PROVENANCE.md."

# insert_notice вставляет NOTICE_LINE после первого идентификатора лицензии в
# первых 12 строках файла, а если его там нет — первой строкой. Тем же
# правилом строка ставится и в дереве фундамента: разойдись правила — шаг 6
# покраснеет на каждом изменённом файле.
insert_notice() {
    local f="$1" at
    at=$(head -n 12 -- "$f" | grep -n -m1 '^// SPDX-License-Identifier:' | cut -d: -f1)
    [ -n "$at" ] || at=0
    awk -v at="$at" -v line="$NOTICE_LINE" \
        'NR == 1 && at == 0 { print line } { print } NR == at { print line }' "$f" > "$f.notice" \
        && mv -- "$f.notice" "$f"
}
...
# --- наши файлы происхождения: исключаются из сравнения ПО ПУТИ -----------------
# Их в апстриме нет, и сравнивать их не с чем. Исключение — ровно названные пути
# от корня фундамента (манифест и файлы правки записей), а НЕ базовые имена:
# исключение по имени (`diff -x PROVENANCE.md`) прощало бы файл с тем же именем
# на любой глубине поддерева, то есть лишний файл, которого нет ни у источника,
# ни в правке (приёмка F1, F1-47). Замер до правки: посев копии манифеста в
# `internal/oauth2/handler/`, `internal/oauth2deps/errorsx/` и копии файла правки
# в `internal/oauth2/handler/` — каждый давал ЗЕЛЁНЫЙ.
own_paths=("$PROVENANCE_FILE")
for ((i = 0; i < PIN_COUNT; i++)); do
    [ "${PIN_PATCH[$i]}" = "none" ] && continue
    dup=0
    for p in "${own_paths[@]}"; do [ "$p" = "${PIN_PATCH[$i]}" ] && dup=1; done
    [ "$dup" -eq 0 ] && own_paths+=("${PIN_PATCH[$i]}")
done
...
for ((i = 0; i < PIN_COUNT; i++)); do
    ...
    # --- 8. уведомление об изменении — последний шаг преобразования --------
    # Изменённым считается файл `.go`, не совпавший побайтово с файлом
    # ИСТОЧНИКА (кэш модулей, до преобразования), и файл, перенесённый из
    # источника (`moved-from`): у него тоже чужой текст с нашими правками.
    # Написанный для фундамента (`written`) — наш целиком, уведомлять не о чем.
    pristine="$src_dir"
    [ "$sub" = "." ] || pristine="$src_dir/$sub"
    noticed=0
    while IFS= read -r -d '' g; do
        rel="${g#./}"
        if [ -e "$pristine/$rel" ]; then
            cmp -s -- "$rebuilt/$rel" "$pristine/$rel" && continue
        elif [ "${added_class["$local_path/$rel"]:-}" != "moved-from" ]; then
            continue
        fi
        insert_notice "$rebuilt/$rel" || die_unavailable "не вставить уведомление об изменении в $rel"
        noticed=$((noticed + 1))
    done < <(cd "$rebuilt" && find . -name '*.go' -type f -print0)
    echo "   уведомл. : строка об изменении вставлена в $noticed файлов .go"

    # Собственные файлы происхождения сравнению не подлежат: их в апстриме нет.
    # Снимаются с КОПИИ области ровно по своим путям (own_paths выше); файл с
    # тем же именем на иной глубине остаётся в сравнении и, если его нет ни у
    # источника, ни в правке, даёт КРАСНЫЙ с его путём.
    #
    ...
    compared="$work/tree-$i"
    mkdir -p "$compared/$(dirname "$local_path")" || die_unavailable "не создать рабочий каталог"
    cp -r "$local_path" "$compared/$local_path" 2>/dev/null || die_unavailable "не скопировать $local_path"
    excluded_here=0
    for own in "${own_paths[@]}"; do
        case "$own" in
            "$local_path"/*) rm -f -- "$compared/$own"; excluded_here=$((excluded_here + 1)) ;;
        esac
    done
    echo "   своё     : исключено по пути $excluded_here"
    delta=$(cd "$compared" && diff -ruN "$rebuilt" "$local_path" 2>&1)
    if [ -n "$delta" ]; then
