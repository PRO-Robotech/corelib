...
#     коммитов не до правила, номер ветки у слияний первой цепочки головы-номера;
...
say "== правило запроса (T0 $(git_rule_t0_text)): голова «$head», коммитов в диапазоне $GIT_RULE_SEEN, из них после правила (судимых формой) $GIT_RULE_AFTER_RULE, номеров сверено с трекером $((checked - ${#unasked[@]})) из $checked"
