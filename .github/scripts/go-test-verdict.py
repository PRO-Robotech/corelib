...
ИСХОДОВ ТРИ, и они не смешиваются:
  ...
  0 — ЗЕЛЁНЫЙ: исполнена хотя бы одна проба, отказов нет;
  1 — КРАСНЫЙ: упала проба либо пакет не собрался;
  2 — БЕСПРЕДМЕТНО: обход пуст — не осмотрено НИ ОДНОГО пакета, — либо пакеты
      осмотрены, а не исполнено НИ ОДНОЙ пробы (их нет или все пропущены). Это
      не успех: «ноль отказов» здесь означало бы «ноль исполненного», а такой
      вердикт нельзя ни подтвердить, ни опровергнуть. Пакет без проб рядом с
      исполненными законен: беспредметность — свойство прогона, не пакета.
...
def verdict(stream, out=sys.stdout):
    ...
    if executed == 0:
        print("", file=out)
        print("БЕСПРЕДМЕТНО: пакетов осмотрено %d, а проб не исполнено НИ ОДНОЙ"
              " (пропущено %d)." % (len(seen_pkgs), test["skip"]), file=out)
        print("«Отказов 0» здесь означало бы «ноль исполненного»: спросить было не у кого.",
              file=out)
        return 2
...
def self_test():
    ...
    # (+) пакеты осмотрены, а проб не исполнено ни одной — БЕСПРЕДМЕТНО: так
    # голый `go test` печатает `ok` на дереве без проб, и числом это «0 отказов
    # из 0 исполненных». Пакет без проб даёт событие `skip` пакета.
    run(2, "(+) пакеты есть, проб нет — беспредметно, не зелёное",
        '{"Action":"skip","Package":"p"}\n'
        '{"Action":"skip","Package":"q"}\n')

    # (−) законный близнец: пакет без проб РЯДОМ с пакетом, где проба
    # исполнена, — зелёное: пакет без проб законен, пока пробы есть где-то ещё.
    run(0, "(−) пакет без проб рядом с исполненной пробой — зелёное",
        green + '{"Action":"skip","Package":"q"}\n')

    # (+) все пробы пропущены — тоже ноль исполненного, а не проход; пропуск
    # при этом назван своим числом и в исполненные не зачтён.
    text = run(2, "(+) все пробы пропущены — беспредметно, не зелёное",
    ...
    # (−) близнец ровно в один факт: та же пропущенная проба и одна
    # исполненная — зелёное, пропуск назван числом.
    text = run(0, "(−) одна исполнена, одна пропущена — зелёное, пропуск назван",
               '{"Action":"pass","Package":"p","Test":"TestB"}\n'
               '{"Action":"skip","Package":"p","Test":"TestA"}\n'
               '{"Action":"pass","Package":"p"}\n')
    if text and ("ПРОПУЩЕНО         : 1" not in text or "проб исполнено    : 1" not in text):
        print("  ПРОВАЛ близнец: исполненная и пропущенная не названы раздельно", file=sys.stderr)
        failed += 1
