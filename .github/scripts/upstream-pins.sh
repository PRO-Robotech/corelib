...
#
# ЗАЧЕМ ОТДЕЛЬНЫЙ РАЗБОРЩИК. Пины апстрима лежат в ОДНОМ месте — блоке
# «```upstream-pins» файла `internal/oauth2/PROVENANCE.md`, — и читаются ОДНОЙ
# функцией. Из `oauth2-provenance.sh` она вынесена затем, чтобы любой
# следующий читатель пинов брал их тем же разбором, а не своей копией: две
# копии разошлись бы после первого подъёма версии, и один гейт судил бы новую
# версию, а другой старую, оба зелёные.
#
