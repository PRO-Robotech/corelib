...
func TestDeferralFormsAreHandedOutAsACopy(t *testing.T) {
	t.Parallel()
	before := treehygiene.DeferralForms()
	if len(before) == 0 {
	...
	after := treehygiene.DeferralForms()
	if after[0] != before[0] {
	...
	seeds[0] = "подмена"
	if treehygiene.DeferralSeeds()[0] == "подмена" {
		t.Fatal("правка отданного среза затравок уехала в перечень разбора")
	}
	t.Logf("осмотрено: форм %d, затравок %d", len(before), len(seeds))
}
